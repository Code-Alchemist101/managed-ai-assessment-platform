...
3. Perform the signed-in provider sanity check in managed Edge
4. Investigate whether the `sequence_gap_detected` flags in the latest human run are a telemetry defect or an acceptable policy downgrade
5. Freeze local v1 after those items are understood
